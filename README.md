...
```
第一步 任务编排 (大模型主导)     工件+规格 → 子任务（去哪磨 + 用哪条带 + 磨掉多少），不含点
   │   inspect_workpiece 看懂几何(面/棱) → 大模型按 region_id 逐区域 add_step
   │   代码只做兜底：区域非空、带序粗→精、参数合法（decompose 给可选的默认建议）
...
| `grinding_inspect_workpiece` | ① | **看懂几何**：点云→候选区域（面/棱）+region_id，供大模型定「去哪磨」 |
| `grinding_add_step` | ① | **大模型主导**：按 region_id 逐区域下子任务（带+去除量），代码兜底校验 |
| `grinding_decompose` | ① | 算法兜底：自动给一个默认拆解建议（带序+分配），大模型可覆盖 |
